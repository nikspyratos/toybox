<x-action-section>
    ...
    <x-slot name="content">
        <div class="p-4 text-sm font-medium text-red-500 rounded border-l-4 border-red-600 dark:border-red-700 bg-red-500/10 dark:bg-red-500/5">
                        ...
                        @if (! is_null($account))
                            <div class="flex items-center space-x-6">
                                @if (Laravel\Jetstream\Jetstream::managesProfilePhotos() && ! is_null($account->avatar_path))
                                    <button class="text-sm text-gray-500 cursor-pointer dark:text-gray-400 hover:text-gray-700 focus:outline-none ms-6 dark:hover:text-gray-300" wire:click="setAvatarAsProfilePhoto({{ $account->id }})">
